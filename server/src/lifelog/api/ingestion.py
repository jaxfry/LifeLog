...
async def ingest_raw_log(
    *,
    ...
    # Use service layer to create the raw log with idempotency support
    db_raw_log, is_new = await IngestionService.create_raw_log(
        session,
        cast(int, actor.id),
        log_in.data,
        device_id=device.id,
        external_id=log_in.external_id or log_in.idempotency_key,
    )
    ...
    if not is_new:
        logger.info(
            f"Duplicate raw_log detected and ignored: source={log_in.source_actor_slug}, "
            f"device_id={device.id}, external_id={log_in.external_id or 'fingerprint'}"
        )

    # TODO: Implement proper async task queue (Celery, RQ, or similar)
    # Background processing is disabled to prevent connection pool exhaustion
    # Processing should be triggered manually via /processing/trigger endpoint
    # or by a separate worker process with its own connection pool
    ...
    #         logger.error("Background processing task failed for raw_log_id=%s: %s", raw_log_id, e, exc_info=True)
    #
