...
async def main():
    ...
    dummy_content = b"This is a test file for LifeLog AI analysis. It contains keywords like Python, FastAPI, and Gemini."
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.post("/api/v1/files/upload", files=files, data={"category": "document", "tags": "test,ai"}, headers={"Host": "localhost"})
...
async def inspect_db(file_id):
    ...
    async with async_session() as session:
        ...
        print(f"Embedding Model: {file.embedding_model}")
        print(f"Embedding Length: {len(file.embedding) if file.embedding is not None else 0}")
        
        print("\nAI Metadata:")
        import json
        print(json.dumps(file.ai_metadata, indent=2))
        
        if file.is_processed and file.embedding is not None:
