...
from app.models.data import RawLog, Event, Session, Timeline
...
async def check_counts():
    async for session in get_session():
        ...
        res = await session.execute(select(func.count()).select_from(Timeline))
        ...
        if timeline_count > 0:
            res = await session.execute(select(Timeline).order_by(Timeline.start_time.desc()).limit(5))
