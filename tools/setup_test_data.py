...
TIMELINE_SRC = TEST_DIR / "2025-05-22.parquet"
...
def copy_test_files() -> None:
    """Copy test Parquet and summary files into storage."""

    if TIMELINE_SRC.exists():
        shutil.copy(TIMELINE_SRC, CURATED_DIR / TIMELINE_SRC.name)
        print(f"Copied timeline data to {CURATED_DIR}")
    else:
        print("Timeline test data not found", file=sys.stderr)
